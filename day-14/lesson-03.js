...
// 자바스크립트에서 this는 "어디서 선언되었는가"가 아니라 "어떻게 호출되었는가"에 따라 결정됩니다.
...
// timer.timout()이 호출될 때, timeout 메서드 안에 this는 timer 객체를 가리킵니다.
...
// 그냥 일반 함수처럼 단독으로 실행됩니다.

// 일반 함수가 단독으로 호출되면, 그 안의 this는 전역 객체(window나 global)을 가리키게 됩니다.
// 전역 객체에는 name 속성이 없기 때문에, this.name는 결국 undefined가 됩니다.
// 즉, 콜백 함수가 실행될 때 timer와의 연결이 끊어버린 것이죠.

// 일반 함수는 호출될 때의 문맥(context)에 따라 this가 결정되는데
// setTimeout은 콜백을 특정 객체의 메서드로 호출하지 않습니다.

// 따라서 이 콜백의 this는 기본적으로 전역 객체(window or global)를 가리키며,
...
// 그 안의 this는 timeout 메서드의 this, 즉 timer2 객체를 그대로 물려 받습니다.
...
// setTimeout은 자바스크립트의 함수(Function)는 맞습니다.
...
const user = new User("안녕하세요", "진짜 안녕할까요?");
console.log(user);
